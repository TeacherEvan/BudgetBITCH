...
export function useBudgets() {
  ...
  useEffect(() => {
    ...
    const dummyBudgets: BudgetCategory[] = [
      { category: 'food', monthlyLimit: 10000, alertAtPct: 80 },
      { category: 'transport', monthlyLimit: 5000, alertAtPct: 80 },
      { category: 'entertainment', monthlyLimit: 3000, alertAtPct: 80 },
    ];
...
export function useNetWorth() {
  const [snapshot, setSnapshot] = useState<{ assets: any[]; liabilities: any[] } | null>(null);
  const [loading, setLoading] = useState(true);
