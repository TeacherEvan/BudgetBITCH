...
describe('useVicinityFeeds', () => {
  ...
  it('fetches and caches vicinity feeds', async () => {
    ...
    const { result } = renderHook(() => useVicinityFeeds('en'));
  ...
  it('falls back to cache on error', async () => {
    const cachedItems = [
      { title: 'Cached News', link: 'https://cached.com', pubDate: new Date().toISOString(), source: 'Cached', category: 'finance', locale: 'en' },
    ];

    // Pre-populate cache
    localStorage.setItem('bb:vicinityNewsCache', JSON.stringify({ items: cachedItems, timestamp: Date.now(), locale: 'en' }));
